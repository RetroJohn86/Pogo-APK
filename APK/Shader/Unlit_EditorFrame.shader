...
void main()
{
    SV_Target0 = texture2D(_texture, vs_TEXCOORD0.xy);
    gl_FragDepth = 1.0;
    return;
}
...
uniform 	float _depthScaleMax;
UNITY_LOCATION(0) uniform mediump sampler2D _texture;
UNITY_LOCATION(1) uniform mediump sampler2D _textureDepthSuppressionMask;
...
void main()
{
    ...
    if(u_xlatb0){
        ...
        u_xlat0.x = u_xlat10_0 * u_xlat1 + _depthScaleMin;
        u_xlat1 = (-u_xlat0.x) * _ZBufferParams.w + 1.0;
        u_xlat2 = dot(u_xlat0.xx, _ZBufferParams.zz);
        gl_FragDepth = u_xlat1 / u_xlat2;
        u_xlat0.x = float(1.0) / u_xlat0.x;
        SV_Target0.xyz = u_xlat0.xxx * vec3(0.25, 0.25, 0.25);
        SV_Target0.w = 1.0;
    } else {
        SV_Target0 = texture2D(_texture, vs_TEXCOORD0.xy);
        gl_FragDepth = 1.0;
